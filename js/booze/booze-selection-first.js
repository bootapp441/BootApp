    ...
    $(document).ready(async function () {
        ...
        async function driveFirstEmptyCar(firstCarIndex = 0) {
            ...
            for (let i = firstCarIndex; i < totalCarsOnPage; i++) {
                ...
                if (carDiv.length) {
                    ...
                    // Check if the car is empty and not in transit
        ...
        async function driveFirstFullCar() {
            ...
            for (let i = 0; i < totalCarsOnPage; i++) {
                ...
                if (carDiv.length) {
                    ...
                    if (isCarMatchingBoozeAndCapacity(boozeLabels) && !isInTransit()) {
                        if (!isCarDriving()) {
                            await executeWithDelay(async () => {
                                goDrive();
                            });
        ...
        async function driveToTopUpCar(firstCarIndex = 0) {
            ...
            for (let i = firstCarIndex; i < totalCarsOnPage; i++) {
                ...
                if (carDiv.length) {
                    ...
                    if (boozeLabels.includes(boozeName) && isPartiallyFilled && !isInTransit()) {
        ...
        var selectionOptions = `
            <div style="display: flex; gap: 20px; justify-content: center; align-items: center; font-family: 'Courier New', Courier, monospace;">        
                <span style="font-size: 9pt; background-color: rgba(161, 45, 19, 0.58); color: white; font-weight: bold; padding: 0px 3px; margin-top: 5px; margin-bottom: 1px; border: 1px solid white; border-radius: 3px; cursor: pointer;" 
                    class="get-first-empty">
                    ▢ EMPT ▢▢▢▢▢
                </span>
                <span style="font-size: 9pt; background-color: rgba(161, 45, 19, 0.58); color: white; font-weight: bold; padding: 0px 3px; margin-top: 5px; margin-bottom: 1px; border: 1px solid white; border-radius: 3px; cursor: pointer;" 
                    class="get-first-topup">
                    ▣ PART ▣▣▣▢▢
                </span>
                <span style="font-size: 9pt; background-color: rgba(161, 45, 19, 0.58); color: white; font-weight: bold; padding: 0px 3px; margin-top: 5px; margin-bottom: 1px; border: 1px solid white; border-radius: 3px; cursor: pointer;" 
                    class="get-first-full">
                    ▣ FULL ▣▣▣▣▣
                </span>
            </div>
            `;
               
        // Change the text "Purchase" to "Start driving car ..."
        $('.sub3').first().contents().filter(function () {
            return this.nodeType === 3 && this.nodeValue.trim() === "Purchase";
        }).each(function () {
            this.nodeValue = "Start driving car ... ";
        });

        $('.sub3').first().append(selectionOptions);

        $('.get-first-empty').click(async function () {
        ...
        $('.get-first-full').click(async function () {
        ...
        $('.get-first-topup').click(async function () {
